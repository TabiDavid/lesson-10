...
fetch(apiURL)
  ...
  .then((jsObject) => {
    ...
    document.getElementById('current-temp').textContent = jsObject.main.temp + " ˚F"; //Current temperature in Fahrenheit
    document.getElementById('humidity').textContent = jsObject.main.humidity + " %"; //Hum1n1d1ty
    document.getElementById('wind-speed').textContent = jsObject.wind.speed + " mph"; //W1nd Sp33d
    ...
    if (t <= 50 && s > 3) {
      ...
      document.getElementById("windchill").textContent = chill.toFixed(2) + " ˚F"; //W1nd Ch177
    } else {
